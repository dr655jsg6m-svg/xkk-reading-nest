...
export function App() {
  ...
  async function lookAtNovel(
    ...
    try {
      ...
      setToast(
        mode === "context"
          ? `已同步${sessionBundle.session.userCurrentPosition.label}，哥哥正在看这里。`
          : "已用兼容模式发送当前段落。"
      );
    } finally {
  ...
  async function confirmSyncBatch() {
    ...
    if (batch.isFinal) {
      if (confirmed.mode === "live_reading") {
        ...
        setToast(`已确认哥哥读到第 ${batch.rangeEnd} 段。`);
        return;
      }
    ...
    <div className="app">
      ...
      {screen === "setup" ? (
        <main className="setup-shell">
          ...
          {existingSession ? (
            <section className="setup-companion-summary" aria-label="哥哥最近短评">
              <div>
                <strong>哥哥最近短评</strong>
                <span>重新导入正文后，陪读 Dock 会继续显示这些短评。</span>
              </div>
              {companionLoading ? <p>正在看看哥哥留下了什么……</p> : null}
